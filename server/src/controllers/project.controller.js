...
export async function addMember(request, response) {
  ...
  if (alreadyMember) {
    throw new HttpError(409, "User is already in this project");
  }

  request.project.members.push({ user: user._id, role: request.body.role });
  await request.project.save();
  await request.project.populate("members.user", "name email avatar");
  response.status(201).json({ project: request.project });
}
...
export async function removeMember(request, response) {
  ...
  await Task.updateMany({ project: request.project._id, assignee: member.user._id }, { $unset: { assignee: "" } });
  member.deleteOne();
  await request.project.save();
  await request.project.populate("members.user", "name email avatar");
  response.json({ project: request.project });
